...
Bundle::Bundle(const unsigned char* buf, const int size)
{
	if (!importData(buf, size))
		throw std::invalid_argument(string(__PRETTY_FUNCTION__) + " : buf is in invalid format.");
}
...
int Bundle::getArray(string *& arr)
{
	...
	if (type != TYPE_STRING)
		throw std::invalid_argument(string(__func__) + " Invalid argument type");
...
{
	int _count = checkValidity(((const unsigned char *)this->buf) + rind, wind - rind);
	return _count;
}
...
void Bundle::get(string & s)
{
	...
	if (type != TYPE_STRING || count != 1)
		throw std::runtime_error(string(__PRETTY_FUNCTION__) + " Invalid operation");
